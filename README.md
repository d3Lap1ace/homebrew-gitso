...
> `sugit` is a CLI tool that clones any GitHub repository (HTTPS **or** SSH) and files it under a predictable tree by owner or domain name 
...
## Usage examples
      ```bash
      sugit git@github.com:d3Lap1ace/homebrew-gitso.git
      sugit https://github.com/d3Lap1ace/homebrew-gitso.git

      # clone into ~/Code
      sugit -d ~/Code git@github.com:d3Lap1ace/homebrew-gitso.git

      # clone dev branch
      sugit -b dev https://github.com/d3Lap1ace/homebrew-gitso.git
      ```
...
1. **Linux**<br>
    * **amd64**<br>
   Download `sugit_linux_amd64.tar.gz`
      ```bash
      tar -zxvf sugit_linux_amd64.tar.gz sugit \
      && cd sugit \
      && sudo chmod +x sugit \
      && sudo mv sugit/sugit /usr/local/bin \
      && sugit --help
      ```
    * **arm64**<br>
   Download `sugit_linux_arm64.tar.gz`
      ```bash
      tar -zxvf sugit_linux_arm64.tar.gz sugit \
      && cd sugit \
      && sudo chmod +x sugit \
      && sudo mv sugit/sugit /usr/local/bin \
      && sugit --help
...
3. **Windows**
    * **amd64**
        1. Download `sugit_windows_amd64.zip`
        2. Unzip `sugit.exe` into a directory on `%PATH%` (e.g. `C:\Tools\`)
        3. Open a new PowerShell / CMD and run `sugit --help`
...
> • remove quarantine tags commend:`sudo xattr -d com.apple.quarantine /usr/local/bin/sugit`<br>
...
> `sugit config --dest <your file directory>`
---
...
> `sugit` 是一个 CLI 工具，可克隆任意 GitHub 仓库（HTTPS **或** SSH），根据域名作者名并将其按可预测的目录树存放到  
> `~/Documents/GitHub.com/<owner>/<repo>`。  
...
3. **Windows**
    * **amd64**
        1. 下载 `sugit_windows_amd64.zip`
        2. 将 `sugit.exe` 解压至 `%PATH%` 中的某个目录（例如 `C:\Tools\`）
        3. 打开新的 PowerShell / CMD 并运行 `sugit --help`

> **提示**  
> • 检测 CPU 架构: `uname -m` (Linux/macOS) / `wmic os get osarchitecture` (Windows)  
> • macOS 通过 平台的二进制文件 安装时需移除 quarantine 标记<br>
> • 移除命令：`sudo xattr -d com.apple.quarantine /usr/local/bin/sugit`<br>
> • macOS 通过 Homebrew 安装时会自动移除 quarantine 标记，无需额外操作  
> • 所有平台的二进制文件均可在每个 GitHub Release 页面获取。

## 修改默认设置
> `sugit config --dest <你的文件目录>`
